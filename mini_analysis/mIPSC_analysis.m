...
save_file_name = 'MINIANALYSIS_210218.mat';

%this needs to be copied from excise_x_points
excised_filename = {'excised_210218.mat'};
...
tRISE = 0.0015*samplert;

tDECAY = 0.01*samplert;

num_zeros_temp_BL = 15;
        ...
        if isempty(cell_id{1,ci}) == 1
            ...
            for kk = 1:cell_id{1,ci}(end,1)
               ...
               if isempty(filename_bycell{1,ci}{kk}) == 1
                ...
                samprate = 10000; %raw data sample rate
                ...
                for ii = 1:length(aDAT) - n % difference between data and template
                    ...
                    %amp cutoff for templates are 5 and 1000
                    if abs(fitted_template(1,1)-fitted_template(1,pk_ind)) > 5 && abs(fitted_template(1,1)-fitted_template(1,pk_ind)) < 1000
                ...
                timeindx(timeindx >= max(timeindx)-350) = NaN; 
                ...
                for i = 1:numel(timeindx)
                    ...
                    if sm_pk_val(i) == pt3_avg(end,i)
                        ...
                        if isnan(sm_pk_ind(i)) == 0 && isempty(event_start_ind{i})==0
                            if event_start_ind{i} >= numptsBL+50
                                sm_pk_val(i) = NaN;
                                sm_pk_ind(i) = NaN;
                    ...
                    if isnan(sm_pk_ind(i)) == 0 && (event_start_aDAT_ind-numptsBL+1) > 0
...
if result == 1
        ...
        save(save_file_name,'AMP_ALL','WAVG','FRQ','wavgs_raw',...
            ...
            'DECAY_INDICES','PT3_DECAY_VALUES','TOTAL_TIME', 'DECAY',...
            'mini_count','meanAmp')
